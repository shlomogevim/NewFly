...
class VideoRecorder(
    ...
    private fun setupMediaRecorder() {
        ...
        mediaRecorder?.apply {
            ...
            setVideoEncodingBitRate(biteRate)
    ...
    private fun buildFile(): File? {
        ...
        val path =
            Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES)?.absolutePath +
                    "/TruOutFurnitue/${date}_video.mp4"
        val file = File(path)
    ...
    private fun buildFileApi29(): FileDescriptor? {
        ...
        val uri=avtivity.contentResolver.insert(MediaStore.Video.Media.EXTERNAL_CONTENT_URI,contentValues)
        return avtivity.contentResolver.openFileDescriptor(uri?:return null,"w")?.fileDescriptor
    ...
    private fun startRecordig() {
        try {
            setupMediaRecorder()
        }catch (e:IOException){
            Toast.makeText(avtivity,"Error recording this video",Toast.LENGTH_LONG).show()
        }
